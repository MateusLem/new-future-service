...
public class AssignmentService {
    ...
    public void create(AssignmentDTO assignmentDTO) {
        ...
        if(!userRepository.existsById(assignmentDTO.getUserId())){
            throw new EntityNotFoundException("User not found with id: " + assignmentDTO.getUserId());
        }
    ...
    public void update(String id, AssignmentDTO assignmentDTO) {
        ...
        if (!assignmentRepository.existsById(assignmentDTO.getId())) {
            throw new EntityNotFoundException("Assignment not found with id: " + assignmentDTO.getId());
        }

        if (assignmentDTO.getUserId() != null || assignmentDTO.getTrailId()!=null){
            throw new IllegalArgumentException("Trails and Users cannot be unnassigned from each other. " +
                    "You must create a new assignment.");
        }

        Assignment existingAssignment = assignmentRepository.findById(assignmentDTO.getId())
                .orElseThrow(() -> new EntityNotFoundException("Assignment not found with id: " + assignmentDTO.getId()));
